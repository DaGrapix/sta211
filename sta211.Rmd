...
MCMC <- function(theta.0, y, n.iter, a, b, k){
  ...
  for (i in 2:n.iter){
    ...
    #r <- ifelse(N>=N.curr, prod((N.curr+1):N), 1/prod((N+1):N.curr))*((1-pi.curr)^(2*(N-N.curr)))
...
ggs_effective(ggs(model.samples), proportion = FALSE, plot = FALSE)
```
